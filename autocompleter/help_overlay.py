"""Floating help panel for global shortcuts."""
...
class HelpOverlayConfig:
    ...
    footer: str = "Esc or Ctrl+/ to close"
    font_size: int = 13
...
def _format_hotkey(hotkey: str) -> str:
    ...
    key = ""
    for part in parts:
        if part in _MODIFIER_GLYPHS:
            modifiers.append(_MODIFIER_GLYPHS[part])
        elif part in _KEY_DISPLAY:
            key = _KEY_DISPLAY[part]
        else:
    ...
    *,
    ...
    return [
        (
            "Generate",
            [
                HelpEntry(_format_hotkey(trigger_hotkey), "Trigger suggestions (or dismiss)"),
                HelpEntry(_format_hotkey(f"shift+{trigger_hotkey}"), "Toggle auto-trigger on/off"),
                HelpEntry(_format_hotkey(regenerate_hotkey), "Regenerate with fresh sampling"),
            ],
        ),
        (
            "Navigate & accept",
        ...
        (
            "Help & feedback",
            [
                HelpEntry(_format_hotkey(help_hotkey), "Show or hide this help panel"),
                HelpEntry(_format_hotkey(report_hotkey), "Capture a bug report for the current app (no content sent)"),
            ],
        ),
    ]
...
if HAS_APPKIT:
    ...
    class HelpOverlayView(AppKit.NSView):
        ...
        def drawRect_(self, rect):
            ...
            title_str.drawAtPoint_(AppKit.NSMakePoint(cfg.padding, y_top - title_size.height))
            ...
            for section_title, entries in cfg.sections:
                ...
                section_str.drawAtPoint_(AppKit.NSMakePoint(cfg.padding, y_cursor))
                y_cursor -= 4.0
                ...
                for entry in entries:
                    ...
                    keys_x = cfg.padding + cfg.keys_column_width - keys_size.width
                    ...
                    desc_rect = AppKit.NSMakeRect(desc_x, desc_y, desc_width, desc_size.height + 2)
...
class HelpOverlay:
    ...
    def _compute_height(self) -> float:
        ...
        for _, entries in cfg.sections:
            total += cfg.font_size + 4.0
            total += cfg.row_height * len(entries)
            total += cfg.section_gap
        total += cfg.footer_font_size + 6.0
    ...
    def hide(self) -> None:
        ...
        if self._window is not None:
            ...
            context.setDuration_(0.08)
            context.setCompletionHandler_(_complete)
    ...
    def _install_click_monitor(self) -> None:
        ...
        def _on_mouse_down(event):
            ...
            if not AppKit.NSPointInRect(point, frame):
    ...
    def _create_window(self, frame) -> None:
        ...
        view = HelpOverlayView.alloc().initWithFrame_config_(content_rect, self._config)
